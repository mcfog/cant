(function(){
  ...
  module.exports = Module = (function(){
    ...
    prototype.handleRequest = function(rpath, _arg, cb){
      ...
      fs = require('fs');
      this.rpath = cant.config('localPath') + String.prototype.replace.apply(rpath, this.pattern);
      res.contentType(this.getContentType());
      fs.stat(this.rpath, function(err, stat){
        if (err) {
          return cb();
        }
        cant.cache({
          ...
          var cpath;
          cpath = path.join('./tmp', "cant." + sha1(_this.rpath) + ".dat");
          require('mkdirp')(path.dirname(cpath), function(err){
            ...
            fs.stat(cpath, function(err, cstat){
              ...
              fs.readFile(_this.rpath, function(err, data){
                if (err || !data) {
                  return next(err || new Error);
                }
                _this.handleData(data, function(err, content){
                  if (err) {
                    return next(err);
                  }
